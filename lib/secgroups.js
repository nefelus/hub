...
(function () {
  ...
  secgroups.getRules = function getRules(companyId, projectId, cloudId) {
    if (! companyId) {
      companyId=0;
    }
    if (! projectId) {
    ...
    for (var i = 0; i < secgroups.secgroupsDefs.length; i++) {
      if ((companyId == secgroups.secgroupsDefs[i]['company_id']) &&
          (projectId == secgroups.secgroupsDefs[i]['project_id']) &&
          ((cloudId === undefined) || (cloudId == secgroups.secgroupsDefs[i]['cloud_id'])) ) {
        res.push({
  ...
  secgroups.loadSecGroups = function loadSecGroups(mysqlClient, cb) {
    ...
    try {
      mysqlClient.query(SQL.secgroups, function(err, rows, fields) {
        ...
        cb(null, secgroups.secgroupsDefs.length+' SecGroups loaded');
      });
    } catch (ex) {
