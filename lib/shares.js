...
(function () {
  ...
  shares.getByIds = function getByIds(mysqlClient, RIds, cloudId, cb) {
    ...
    if (RIds.length) {
      ...
      stmt += 'concat(f.LOCAL_IPV4,":",v.MOUNT_POINT,"/",m.DIR) as LOCATION, m.MOUNT_POINT as MOUNT_POINT, m.MOUNT_PARAMS as MOUNT_PARAMS, m.FS_TYPE as FS_TYPE ';
            ...
            for (var i = 0; i < rows.length; i++) {
              ...
              if ((record.location !== '') && (record.mount_point !== '') && (record.uuid !== '') &&
                ...
                RShares.push({'id' : record.id,
                              'fstype' : record.fs_type.toLowerCase(),
