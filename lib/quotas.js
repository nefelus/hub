...
(function () {
    ...
    var QueueStats = function QueueStats() {
      ...
      this.add = function (company, user, project, tool, machine, cloud) {
      ...
      this.resolveLimits = function(opt) {
        ...
        if (opt) {
          ...
          if (opt.cloud) {
            s = opt.cloud;
            // SESSIONS
            candidateOpts = [{'resourceType' : 'S', 'resourceId' : s}, // Total sessions
                             {'company' : c, 'resourceType' : 'S', 'resourceId' : s}, // Total company sessions
                             {'company' : c, 'user' : u, 'resourceType' : 'S', 'resourceId' : s}, // Total user sessions
                             {'company' : c, 'project' : p, 'resourceType' : 'S', 'resourceId' : s}, // Total project sessions
                             {'company' : c, 'user' : u, 'project' : p, 'resourceType' : 'S', 'resourceId' : s}, // Total project/user sessions
                            ];
          }
          if (opt.tool) {
            t = opt.tool;
    ...
    quotas.prepareStatement = function prepareStatement(sessions) {
      ...
      if (sessions.length) {
        ...
        stmt += '(RESOURCE_TYPE="SESSIONS" and RESOURCE_ID in ('+clouds.join(',')+'))';
        stmt += ') ';
    ...
    quotas.loadQueueQuotas = function loadQueueQuotas(mysqlClient, sessions, cb) {
        ...
        if (sqlstmt !== '') {
          mysqlClient.query(sqlstmt, function(err, rows, fields) {
            ...
            if (rows.length !== 0) {
              ...
              quotas.limits.add(0, 0, 0, 'S', 0, 0, true); // SESSIONS
              quotas.limits.add(0, 0, 0, 'T', 0, 0, true); // TOOLS
              quotas.limits.add(0, 0, 0, 'M', 0, 0, true); // MACHINES
            }
    ...
    quotas.checkQueueQuota = function checkQueueQuota(companyId, userId, projectId, toolId, machineId, cloudId, severity, debug) {
      ...
      var used = {
        systemSessions : this.stats.getStats({cloud: cloudId}),
        systemMachine : this.stats.getStats({machine : machineId}),
        systemTool : this.stats.getStats({tool : toolId}),

        companySessions : this.stats.getStats({cloud: cloudId, company : companyId}),
        ...
        userSessions : this.stats.getStats({cloud: cloudId, company : companyId, user : userId}),
        ...
        projectSessions : this.stats.getStats({cloud: cloudId, company : companyId, project : projectId}),
        ...
        userProjectSessions : this.stats.getStats({cloud: cloudId, company : companyId, user : userId, project : projectId}),
      ...
      var limits = {
        systemSessions : this.limits.resolveLimits({cloud: cloudId}),
        systemMachine : this.limits.resolveLimits({machine : machineId}),
        systemTool : this.limits.resolveLimits({tool : toolId}),

        companySessions : this.limits.resolveLimits({cloud: cloudId, company : companyId}),
        ...
        userSessions : this.limits.resolveLimits({cloud: cloudId, company : companyId, user : userId}),
        ...
        projectSessions : this.limits.resolveLimits({cloud: cloudId, company : companyId, project : projectId}),
        ...
        userProjectSessions : this.limits.resolveLimits({cloud: cloudId, company : companyId, user : userId, project : projectId}),
