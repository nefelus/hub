...
(function () {
    ...
    quotashares.prepareStatement = function (opt) {
      ...
      if ( ! rtype) {
        stmt += ' RESOURCE_TYPE in ("IP_DATA", "IP_DATA_LIB", "USER_DATA", "IP_DOCS", "TOOL_DOCS", "TOOLS_DATA") ';
      } else {
      ...
      try {
        sqlClient.query(stmt, function(err, rows, fields) {
            ...
            for (i = 0; i < rows.length; i++) {
              ...
              switch (resourceType) {
                ...
                case 'TOOL_DOCS':
                  ...
                  if (typeof tmp[r][resourceId] === 'undefined') {
          ...
          for (var key in tmp) {
            ...
            if (typeof rids[quotashares.iResourceTypes[key]] === 'undefined') {
              rids[quotashares.iResourceTypes[key]] = [];
            }
            for (var key2 in tmp[key]) {
              rids[quotashares.iResourceTypes[key]].push({id: key2, perm: tmp[key][key2]});
            }
          }
          cb(null, rids);
        });
    ...
    quotashares.addOrReplaceAdmimMounts = function (aIds, bIds) {
      var retIds = [];
      var i;
      var j;
      retIds = retIds.concat(aIds);
      for (i = 0; i < bIds.length; i++) {
        j = _.findIndex(retIds, bIds[i]);
        if (j > -1) {
          retIds[j].perm = bIds[i].perm;
        } else {
          retIds.push(bIds[i]);
        }
      }
    ...
    quotashares.uniq = function (Ids) {
      ...
      for (i = 0; i < Ids.length; i++) {
        if (b[Ids[i].id]) {
          if (b[Ids[i].id] > Ids[i].perm) {
            b[Ids[i].id] = Ids[i].perm;
          }
        } else {
            b[Ids[i].id] = Ids[i].perm;
        }
      ...
      for (key in b) {
        retIds.push({id: key, perm: b[key]});
      }
      return retIds;
    };

    quotashares.getPermission = function (Ids, id) {
      var ret = null;
      var i;
      for (i = 0; i < Ids.length; i++) {
        if (Ids[i].id == id) {
