...
nt.createCompanyPath = function createCompanyPath(eid) {
  var hash = crypto.createHash('sha1').update('e-'+eid).digest('hex')
  return 'e-'+eid+'-'+hash.substr(0,8)+'/';
}

nt.createUserPath = function createUserPath(eid, uid) {
  var cpath=nt.createCompanyPath(eid);
  var hash = crypto.createHash('sha1').update('u-'+uid).digest('hex')
  return cpath+'users/u-'+uid+'-'+hash.substr(0,8)+'/';
}

nt.createProjectPath = function createProjectPath(eid, pid) {
  var cpath=nt.createCompanyPath(eid);
  var hash = crypto.createHash('sha1').update('p-'+pid).digest('hex')
  return cpath+'projects/p-'+pid+'-'+hash.substr(0,8)+'/';
}

nt.createUserHome = function createUserHome(eid, uid) {
  var cpath=nt.createUserPath(eid, uid);
  return cpath+'Home/';
}

nt.createUserHomeFromSessionId = function createUserHomeFromSessionId(sessionId) {
  var sid;
  var ssid;
  ...
  var cpath=nt.createUserPath(sid.companyId, sid.clientId);
  return cpath+'Home'; // The slash at the end is not added intentionaly
}

nt.createProjectUserPath = function createProjectUserPath(cid, pid, uid) {
  var cpath=nt.createProjectPath(cid, pid);
  var hash = crypto.createHash('sha1').update('u-'+uid).digest('hex')
  return cpath+'users/u-'+uid+'-'+hash.substr(0,8)+'/';
}

nt.createSessionPath = function createSessionPath(sessionId) {
  var sid;
  var ssid;
  if (typeof sessionId === 'string') {
    sid = nt.parseSessionId(sessionId);
  ...
  ssid = sid.companyId+'_'+sid.clientId+'_'+sid.projectId+'_'+sid.toolId+'_'+sid.taskId;
  var cpath=nt.createProjectUserPath(sid.companyId, sid.projectId, sid.clientId);
