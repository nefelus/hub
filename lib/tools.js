...
nt.createToolPath = function createToolPath(tid, installationId) {
  var stuffed = tid;
...
nt.getDateTimeNow = function getDateTimeNow() {

  var withDelim = (typeof arguments[arguments.length-2] !== 'undefined') ? arguments[arguments.length-2] : true;
  var twelve = (typeof arguments[arguments.length-1] !== 'undefined') ? arguments[arguments.length-1] : false;
  var d = arguments[arguments.length-3] || new Date();
  ...
  m = m < 10 ? '0' + m : m;
  s = s < 10 ? '0' + s : s;
  return d.getFullYear().toString() + dd + mo + dd + da + dm + h + dt + m + dt + s + t;
};
...
nt.mkKey = function mkKey(algorithm, params) {
  var _enckey = '';
  ...
  var checksum = crypto.createHash(algorithm);
  var enckey = checksum.update(_enckey).digest('hex');
  return enckey;
};
...
nt.sanitizeCmdlineParams = function sanitizeCmdlineParams(cmd) {
 var cmda = shellQuote.parse(cmd);
 //console.log(cmd);
 //console.log(cmda);
 var i=0, found=-1;
 for (i=0; i < cmda.length; i++) {
   if ( typeof cmda[i] !== 'string') {
     found = i;
     break;
   }
 }
