...
/*
 *
 * TODO: exponential backoff on restart
 *

// Inspired by: https://github.com/waylonflinn/re

  function createExponentialRetry(spec){
    return function(retries){
      var spread = Math.random() + 1;
      var initial = spec.initial;
      var base = spec.base || 2;
      var max = spec.max;
      var full = spread * initial * Math.pow(base, retries);
  ...
  function createLinearRetry(spec){
    return function(retries){
      ...
      var initial = spec.initial;
      var max = spec.max;
      var full = spread*initial*(retries+1);
  ...
  this.checkout = function checkout(cb) {
    ...
    request(preparedRequest.options, function(error, response, body) {
      if (error) {
        ...
        if (self.active === true) {
          self.active = false;
          self.emit('change', self.active);
        }
        if (self.sessionInfo.auto) {
          self.restartTimer = setTimeout(function() {self.checkout(cb);}, self.restartTimeout);
        } else {
          cb(error, false);
        }
      } else {
        ...
        if ((isValid) && (response.statusCode == 200)) {
          try {
            ...
            self.sessionInfo.sessionId = x.sessionId;
            self.saveToFile(x.sessionId);
            self.sessionInfo.refreshInterval = x.refreshInterval;
            if (self.active !== true) {
              self.active = true;
              self.emit('change', self.active);
            }
            self.sessionInfo.refreshTimer = setInterval(function(){self.refresh();}, self.sessionInfo.refreshInterval);
            cb(null, true);
          } catch (e) {
            self.saveToFile('');
            if (self.active === true) {
  ...
  this.checkin = function checkin(cb) {
    ...
    request(preparedRequest.options, function(error, response, body) {
      if (error) {
        ...
        self.sessionInfo.sessionId = null;
        if (self.sessionInfo.refreshTimer !== null) {
          clearInterval(self.sessionInfo.refreshTimer);
        }
        self.sessionInfo.refreshTimer = null;
        self.sessionInfo.refreshInterval = 0;
        if ((isValid) && (response.statusCode == 200)) {
          cb(null, true);
        } else {
